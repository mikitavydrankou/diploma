export const LOGIN_REQUEST = "LOGIN_REQUEST";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAILURE = "LOGIN_FAILURE";

export const REGISTER_REQUEST = "REGISTER_REQUEST";
export const REGISTER_SUCCESS = "REGISTER_SUCCESS";
export const REGISTER_FAILURE = "REGISTER_FAILURE";

export const LOGOUT = "LOGOUT";

export const CREATE_OFFER_REQUEST = "CREATE_OFFER_REQUEST";
export const CREATE_OFFER_SUCCESS = "CREATE_OFFER_SUCCESS";
export const CREATE_OFFER_FAIL = "CREATE_OFFER_FAIL";

export const DELETE_OFFER_REQUEST = "DELETE_OFFER_REQUEST";
export const DELETE_OFFER_SUCCESS = "DELETE_OFFER_SUCCESS";
export const DELETE_OFFER_FAIL = "DELETE_OFFER_FAIL";

export const OFFERS_FETCH_REQUEST = "OFFER_FETCH_REQUEST";
export const OFFERS_FETCH_SUCCESS = "OFFER_FETCH_SUCCESS";
export const OFFERS_FETCH_FAIL = "OFFER_FETCH_FAIL";

export const OFFER_FETCH_REQUEST = "OFFER_FETCH_REQUEST";
export const OFFER_FETCH_SUCCESS = "OFFER_FETCH_SUCCESS";
export const OFFER_FETCH_FAIL = "OFFER_FETCH_FAIL";
